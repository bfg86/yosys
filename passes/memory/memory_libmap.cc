...
struct Parser {
	...
	void parse_ram_item() {
		...
		if (token == "ifdef") {
			...
			dims.widthscale = false;
			if (peek_token() == "widthscale") {
				get_token();
				dims.widthscale = true;
			}
			get_semi();
...
struct MemMapping {
	...
	void emit_port(const MemConfig &cfg, std::vector<Cell*> &cells, const PortGroupDef &pdef, int pi, int wpidx, int rpidx, const std::vector<int> &hw_addr_swizzle);
		...
		for (int base_width_log2 = start_base; base_width_log2 < GetSize(dims.dbits); base_width_log2++) {
			...
			while (1) {
				...
				if (min_width_ok) {
					...
					if (dims.widthscale)
						cost = dims.cost * mult_a * mult_wide * mem.width / unit_width * cfg.repl_port;
...
void MemMapping::emit_port(const MemConfig &cfg, std::vector<Cell*> &cells, const PortGroupDef &pdef, int pi, int wpidx, int rpidx, const std::vector<int> &hw_addr_swizzle) {
	auto &rdef = lib.ram_defs[cfg.ram_def];
	auto &dims = rdef.dims[cfg.dims_def].val;
	const char *pname = pdef.names[pi].c_str();

	if (wpidx == -1 && rpidx == -1) {
		for (auto cell: cells)
			cell->setParam(stringf("\\PORT_%s_USED", pname), false);
		return;
	}

	for (auto cell: cells) {
		cell->setParam(stringf("\\PORT_%s_USED", pname), true);
		if (pdef.kind == PortKind::Srsw || pdef.kind == PortKind::Arsw) {
			cell->setParam(stringf("\\PORT_%s_WR_USED", pname), wpidx != -1);
			cell->setParam(stringf("\\PORT_%s_RD_USED", pname), rpidx != -1);
		}
	}
	...
	if (wpidx != -1) {
		...
		for (auto &it: wpcfg.portopts)
			for (auto cell: cells)
				cell->setParam(stringf("\\PORT_%s_OPTION_%s", pname, it.first.c_str()), it.second);
		clk = wport.clk;
	...
	if (pdef.kind != PortKind::Ar) {
		switch (clkpol_kind) {
			...
			case ClkPolKind::Anyedge:
				for (auto cell: cells)
					cell->setParam(stringf("\\PORT_%s_CLKPOL", pname), clk_pol);
		}
		for (auto cell: cells)
			cell->setPort(stringf("\\PORT_%s_CLK", pname), clk);
	}
	...
	if (!dims.tied) {
		for (auto cell: cells) {
			if (wdef.tied) {
				cell->setParam(stringf("\\PORT_%s_WIDTH", pname), dims.dbits[hw_wr_wide_log2]);
			} else {
				if (wpidx != -1)
					cell->setParam(stringf("\\PORT_%s_WR_WIDTH", pname), dims.dbits[hw_wr_wide_log2]);
				if (rpidx != -1)
					cell->setParam(stringf("\\PORT_%s_RD_WIDTH", pname), dims.dbits[hw_rd_wide_log2]);
			}
		}
	}
	...
	for (int i = 0; i < hw_wr_wide_log2 && i < hw_rd_wide_log2; i++)
		hw_addr[i] = State::S0;
	for (auto cell: cells)
		cell->setPort(stringf("\\PORT_%s_ADDR", pname), hw_addr);
	...
	if (wpidx != -1) {
		...
		for (int rd = 0; rd < cfg.repl_d; rd++) {
			...
			cell->setPort(stringf("\\PORT_%s_WR_DATA", pname), hw_wdata);
			cell->setPort(stringf("\\PORT_%s_WR_EN", pname), hw_wren);
			if (cfg.byte != 0)
				cell->setParam(stringf("\\PORT_%s_WR_EN_WIDTH", pname), GetSize(hw_wren));
		}
	}
	...
	if (rpidx != -1) {
		...
		for (int rd = 0; rd < cfg.repl_d; rd++) {
			auto cell = cells[rd];
			if (rpcfg.emit_en) {
				cell->setPort(stringf("\\PORT_%s_RD_EN", pname), rport.en);
			}
			if (rport.arst != State::S0) {
				cell->setPort(stringf("\\PORT_%s_RD_ARST", pname), rport.arst);
				cell->setParam(stringf("\\PORT_%s_RD_ARST_USED", pname), true);
			}
			if (rport.srst != State::S0) {
				cell->setPort(stringf("\\PORT_%s_RD_SRST", pname), rport.srst);
				cell->setParam(stringf("\\PORT_%s_RD_SRST_USED", pname), true);
				cell->setParam(stringf("\\PORT_%s_RD_CE_OVER_SRST", pname), rport.ce_over_srst);
			}
			for (auto &it: rpcfg.resetvals) {
				...
				cell->setParam(stringf("\\PORT_%s_%s", pname, it.first.c_str()), val);
			}
			SigSpec hw_rdata = mem.module->addWire(NEW_ID, dims.dbits[hw_rd_wide_log2]);
			cell->setPort(stringf("\\PORT_%s_RD_DATA", pname), hw_rdata);
			SigSpec lhs;
			SigSpec rhs;
		...
		for (int pgi = 0; pgi < GetSize(rdef.ports); pgi++) {
			...
			for (int pi = 0; pi < GetSize(pdef.names); pi++) {
				if (pi < GetSize(ports[rp][pgi]))
					emit_port(cfg, cells, pdef, pi, ports[rp][pgi][pi].first, ports[rp][pgi][pi].second, hw_addr_swizzle);
				else
					emit_port(cfg, cells, pdef, pi, -1, -1, hw_addr_swizzle);
			}
		}
	}
	mem.remove();
